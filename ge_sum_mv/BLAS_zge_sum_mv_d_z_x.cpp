...
void BLAS_zge_sum_mv_d_z_x(enum blas_order_type order,
                           int m,
                           int n,
                           const void *alpha,
                           const double *a,
 ...
 */
{
//static const char routine_name[] = "BLAS_zge_sum_mv_d_z_x";
#if 0
          ...
          for (i = 0, yi = y_starti; i < m; i++, yi += incyi) {
            ...
            for (j = 0, xi = x_starti; j < n; j++, xi += incxi) {
              ...
              b_elem = b_i[bij];
              {
                ...
                compute_doubledouble_eq_double_mul_double(&head_t, &tail_t, b_elem, x_elem[0]);
                head_prod[0] = head_t;
                tail_prod[0] = tail_t;
                compute_doubledouble_eq_double_mul_double(&head_t, &tail_t, b_elem, x_elem[1]);
                head_prod[1] = head_t;
                tail_prod[1] = tail_t;
              }
              {
          ...
          for (i = 0, yi = y_starti; i < m; i++, yi += incyi) {
            ...
            for (j = 0, xi = x_starti; j < n; j++, xi += incxi) {
              ...
              a_elem = a_i[aij];
              {
                /* Compute complex-extra = complex-double * real. */
                double head_t, tail_t;
                compute_doubledouble_eq_double_mul_double(&head_t, &tail_t, a_elem, x_elem[0]);
                head_prod[0] = head_t;
                tail_prod[0] = tail_t;
                compute_doubledouble_eq_double_mul_double(&head_t, &tail_t, a_elem, x_elem[1]);
                head_prod[1] = head_t;
                tail_prod[1] = tail_t;
              }
              {
