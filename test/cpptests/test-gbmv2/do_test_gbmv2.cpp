...
//  incx_gen = 1;
  ...
  if (max_mn * 2 > 0 && x_head == nullptr) {
    BLAS_error("blas_malloc", 0, 0, "malloc failed.\n");
  }
  x_tail = (float *) blas_malloc(max_mn * 2 * sizeof(float));
  if (max_mn * 2 > 0 && x_tail == nullptr) {
    BLAS_error("blas_malloc", 0, 0, "malloc failed.\n");
  }
  ...
  if (max_mn > 0 && x_head_gen == nullptr) {
    BLAS_error("blas_malloc", 0, 0, "malloc failed.\n");
  }
  x_tail_gen = (float *) blas_malloc(max_mn * sizeof(float));
  if (max_mn > 0 && x_tail_gen == nullptr) {
    BLAS_error("blas_malloc", 0, 0, "malloc failed.\n");
  }
                        ...
                        for (incy_val = -2; incy_val <= 2; incy_val++) {
                          ...
                          XBLAS::gbmv2(order_type,
                                       ...
                                       *reinterpret_cast<std::complex<float> *>(alpha),
                                       AB,
                                       lda,
                                       reinterpret_cast<std::complex<float> *>(x_head),
                                       reinterpret_cast<std::complex<float> *>(x_tail),
                                       incx_val,
                                       *reinterpret_cast<std::complex<float> *>(beta),
                                       reinterpret_cast<std::complex<float> *>(y),
                                       incy_val);
...
#endif
{
  ...
  if (nsizes < 0 || ntests < 0 || debug < 0 || debug > 3)
    BLAS_error("Testing gbmv2", 0, 0, nullptr);
