...
void BLAS_dgemm_s_d_x(enum blas_order_type order,
 ...
 */
{
//char *routine_name = "BLAS_dgemm_s_d_x";
#if 0
  XBLAS::gemm_x(order, transa, transb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc, prec);
} /* end BLAS_dgemm_s_d_x */
#else 
            ...
            for (j = 0; j < n; j++, cij += inccij, bj += incbj) {
              ...
              for (h = 0; h < k; h++, aih += incaih, bhj += incbhj) {
                ...
                }
                {
                  ...
                  compute_doubledouble_eq_double_mul_double(&head_prod, &tail_prod, dt, b_elem);
                }
