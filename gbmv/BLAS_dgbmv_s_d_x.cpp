...
void BLAS_dgbmv_s_d_x(enum blas_order_type order,
      ...
      for (i = 0; i < leny; i++) {
        ...
        for (j = ra - la; j >= 0; j--) {
          ...
          a_elem = a_i[aij];
          {
            ...
            compute_doubledouble_eq_double_mul_double(&head_prod, &tail_prod, x_elem, dt);
          }
        ...
        y_elem = y_i[iy];
        compute_doubledouble_eq_double_mul_double(&head_tmp2, &tail_tmp2, beta_i, y_elem);
