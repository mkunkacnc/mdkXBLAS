...
void BLAS_dtpmv_x(enum blas_order_type order,
 ...
 */
{
//char *routine_name = "BLAS_dtpmv_x";
#if 0
          ...
          if ((uplo == blas_upper &&
            ...
            for (matrix_row = 0; matrix_row < n; matrix_row++) {
              ...
              while (col_index < n) {
                vecval = x_i[x_index];
                if ((diag == blas_unit_diag) && (col_index == matrix_row)) {
                  compute_doubledouble_eq_double_mul_double(&head_rowtmp, &tail_rowtmp, vecval, one);
                } else {
                  matval = tp_i[tp_index];
