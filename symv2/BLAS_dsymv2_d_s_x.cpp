...
#include <blas_fpu.h>
void BLAS_dsymv2_d_s_x(enum blas_order_type order,
 ...
 */
{
//char *routine_name = "BLAS_dsymv2_d_s_x";
#if 0
      ...
      for (i = 0, yi = yi0, ai = 0; i < n; i++, yi += incy, ai += incai) {
        ...
        for (j = 0, aij = ai, xi = xi0; j < i; j++, aij += incaij, xi += incx) {
          ...
          x_elem = x_head_i[xi];
          {
            ...
            compute_doubledouble_eq_double_mul_double(&head_prod1, &tail_prod1, a_elem, dt);
          }
          compute_doubledouble_eq_doubledouble_add_doubledouble(&head_sum1, &tail_sum1, head_sum1, tail_sum1, head_prod1, tail_prod1);
          x_elem = x_tail_i[xi];
          {
            double dt = (double) x_elem;
            compute_doubledouble_eq_double_mul_double(&head_prod2, &tail_prod2, a_elem, dt);
          }
